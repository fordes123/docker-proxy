...
export class DockerProxy {
	...
	async fetch(request: Request): Promise<Response> {
		...
		if (this.isBrowserRequest(headers)) {
	...
	private async handleBrowserRequest(request: Request, url: URL): Promise<Response> {
		...
		console.log(`Current mode: ${model}, User-Agent: ${request.headers.get('user-agent')}`);
		...
		switch (model) {
			...
			default:
				return this.handleStaticMode(value, url, request);
		}
	}

	private async handleStaticMode(value: string, url: URL, request: Request): Promise<Response> {
		const htmlHeaders = { 'Content-Type': 'text/html; charset=UTF-8' };

		if (value === 'nginx') {
			return new Response(NGINX_TEMPLATE, { headers: htmlHeaders });
		}

		if (value === 'search') {
			if (url.pathname === '/') {
				return new Response(SEARCH_TEMPLATE, { headers: htmlHeaders });
			}
		...
		return new Response(value, { headers: htmlHeaders });
	}
	...
	private async handleDockerResponse(response: Response, originalUrl: URL): Promise<Response> {
		...
		if (authenticate) {
			headers.set('www-authenticate',
				authenticate.replace(DOCKER_AUTH_HOST, originalUrl.hostname)
			);
		}
	...
	private getTargetRegistry(url: URL): [string, URL] {
		...
		const registryKey = namespace || (subdomain !== url.hostname ? subdomain : 'docker');
		const registryUrl = REGISTRY_ROUTES[registryKey] || REGISTRY_ROUTES.docker;
